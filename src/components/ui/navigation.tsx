...
const Navigation = ({ 
  ...
  return (
    <nav className={clsx("bg-white border-b border-charcoal-200 shadow-soft", className)}>
        ...
        <div className="flex justify-between h-16">
          ...
          <div className="flex items-center">
            <Link href="/" className="flex-shrink-0 flex items-center">
              {logo || (
                <span className="text-xl font-heading font-bold text-sage-600">
                  Wingspan Yoga
                </span>
              )}
            </Link>
          ...
          <div className="hidden md:flex items-center space-x-8">
            {items.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="flex items-center space-x-1 text-charcoal-600 hover:text-sage-600 px-3 py-2 rounded-soft text-sm font-medium transition-colors"
              >
                {item.icon}
            ...
            {user ? (
              <div className="relative">
                <button
                  ...
                  className="flex items-center space-x-2 text-charcoal-600 hover:text-sage-600 px-3 py-2 rounded-soft text-sm font-medium transition-colors"
                >
              ...
              <div className="flex items-center space-x-4">
                ...
                <Link
                  href="/auth/register"
                  className="bg-sage-500 hover:bg-sage-600 text-white px-4 py-2 rounded-soft text-sm font-medium transition-colors"
                >
                  Register
                </Link>
              </div>
            ...
            <div className="px-2 pt-2 pb-3 space-y-1">
              {items.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="flex items-center space-x-2 text-charcoal-600 hover:text-sage-600 hover:bg-charcoal-50 px-3 py-2 rounded-soft text-base font-medium transition-colors"
